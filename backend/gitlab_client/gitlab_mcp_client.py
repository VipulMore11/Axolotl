...
class GitLabMCPClient:
    ...
    async def _get_or_create_client(self, project_id: str) -> Optional[gitlab.Gitlab]:
        ...
        # If project is linked to a user, use their OAuth token.
        # Otherwise, fall back to the PAT stored on the project itself.
        access_token = ""
        token_kwargs = {}
        ...
        if "user_id" in project_config:
            ...
            if user:
                ...
                # We assume tokens from the users collection are OAuth tokens
                token_kwargs = {"oauth_token": access_token}
                ...
                token_kwargs = {"private_token": access_token}
        else:
            access_token = project_config.get("access_token", "")
            token_kwargs = {"private_token": access_token}
        ...
        try:
            ...
            client = gitlab.Gitlab(
                url=project_config.get("gitlab_url", "https://gitlab.com"),
                **token_kwargs
            )

            # Verify connection
            print(f"[DEBUG] Authenticating GitLab client at {project_config.get('gitlab_url', 'https://gitlab.com')}...")
            client.auth()
