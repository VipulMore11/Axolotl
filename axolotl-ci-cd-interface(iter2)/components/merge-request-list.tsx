...
import { mergeRequests, type MergeRequestRecord } from "@/lib/axolotl-data"
...
const statusBadge: Record<MergeRequestRecord["status"], { label: string; cls: string }> = {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-col gap-3">
        {rows.map((mr) => {
          const sb = statusBadge[mr.status]
          return (
            <li
              key={mr.iid}
            ...
            >
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  ...
                  {mr.authorIsAgent && mr.rootCause !== "—" && (
                    <p className="mt-1 text-xs text-muted-foreground">
                      <span className="text-accent">root cause:</span> {mr.rootCause}
                    </p>
                  )}
                ...
                <div className="flex shrink-0 flex-col gap-2 lg:items-end">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span className="font-mono">{mr.openedAt}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                    <span
                      className={`size-1.5 rounded-full ${
                        mr.approvalsGiven >= mr.approvalsRequired ? "bg-chart-3" : "bg-primary animate-pulse"
                      }`}
                    />
                    {mr.approvalsGiven} / {mr.approvalsRequired} approvals
                  </div>
                </div>
              </div>
            </li>
